...
_ETF_MAP = {"TECH": 3, "BLEND": 1, "DIV": 0, "CONC": 0, "LOSS": 2}
_EMPTY_MAP = {"TECH": 0, "BLEND": 0, "DIV": 0, "CONC": 0, "LOSS": 0}


def test_optimize_returns_optimization_result():
    stocks = _make_stocks()
    with patch.object(optimizer, "__get_etf_map", return_value=_ETF_MAP):
...
def test_risk_aware_and_profit_only_are_portfolios():
    stocks = _make_stocks()
    with patch.object(optimizer, "__get_etf_map", return_value=_EMPTY_MAP):
        result = optimizer.optimize(stocks, budget=50.0)
    for portfolio in (result.risk_aware, result.profit_only):
        assert hasattr(portfolio, "allocations")
        assert hasattr(portfolio, "risk_metrics")
...
def test_budget_not_exceeded():
    ...
    budget = 50.0
    with patch.object(optimizer, "__get_etf_map", return_value=_EMPTY_MAP):
        result = optimizer.optimize(stocks, budget=budget)
    for portfolio in (result.risk_aware, result.profit_only):
